...
# Define a function to check if a number is prime num
def is_prime(n):
    if n < 2:
        return False
...
# Define a function to factor a single RSA number using the trial solve
def factor_rsa_number(n):
    ...
    while i <= sqrt(n):
        if n % i == 0:
            ...
            i += 2
    # If n is still greater than 2, it must be prime num
    if n > 2:
        factors.append(n)
